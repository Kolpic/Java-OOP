...
public abstract class MagicianImpl implements Magician{
    ...
    public void takeDamage(int points) {
        ...
        int protectionAfterDamage = protection - points;

        if (protectionAfterDamage <= 0) {
            protectionAfterDamage = Math.abs(protectionAfterDamage);

            int healthPoints = getHealth();
            int healthPointsAfterDamage = healthPoints - protectionAfterDamage;

            setHealth(healthPointsAfterDamage);
        } else {
            setProtection(protectionAfterDamage);
        }
    }
}
